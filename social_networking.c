...
void display_suggestions(int v)
{
	...
	{	
	    ...
	    {
	    	if(marked[i]==0)
			printf("\t%-15s\t\t  %02d\t\tno connection link\n",u[i].user_name,u[i].user_id);
		else
...
int main()
{
    ...
    if(fp2!=NULL)
        ...
        for(i=0;i<n;i++)
        {
            fscanf(fp2,"%s%d%s%s%s%s",u[i].user_name,&u[i].user_id,u[i].password,u[i].place,u[i].profession,u[i].email);
        }
